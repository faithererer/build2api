...
def run_browser_instance(config):
    ...
    # ---------- 仅此处做了“最小改动”：兼容字符串/字典代理，并合并 bypass ----------
    launch_options = {"headless": headless_mode}

    def _normalize_proxy(p):
        """
        接受:
          - 字符串: "http://user:pass@host:port" 或 "host:port"
          - 字典:   {"server":"http://host:port","username":"...","password":"...","bypass":"..."}
        返回 Playwright 需要的 dict；解析失败返回 None
        """
        if not p:
            return None
        if isinstance(p, str):
        ...
        if isinstance(p, dict):
            d = {}
            server = p.get("server") or p.get("url") or p.get("proxy")
            if server:
                if "://" not in server:
                    server = "http://" + server
                d["server"] = server
            ...
            if p.get("bypass"):
                d["bypass"] = p["bypass"]
            return d if "server" in d else None
        return None

    if proxy:
        px = _normalize_proxy(proxy)
        if px:
            # 合并 bypass：优先用配置里的 bypass，再拼 NO_PROXY，再加默认本地直连清单
            default_bypass = "localhost,127.0.0.1,::1,*.local,host.docker.internal"
            env_no_proxy = os.environ.get("NO_PROXY") or os.environ.get("no_proxy")
            merged = ",".join([s for s in [px.get("bypass"), config.get("no_proxy"), env_no_proxy, default_bypass] if s])
            if merged:
                px["bypass"] = merged
            launch_options["proxy"] = px
            logger.info(f"使用代理(server): {px.get('server')} bypass: {px.get('bypass')}")
        else:
            logger.warning("proxy 配置无法解析，已忽略。")
    # -----------------------------------------------------------------------
    ...
    try:
        with Camoufox(**launch_options) as browser:
            ...
            if "accounts.google.com/v3/signin/identifier" in final_url:
                ...
                return
            elif expected_url.split('?')[0] in final_url or "apps/drive" in final_url:
